...
TIMBRES = {
    ...
    "bass": Timbre(
        harmonics=[
            (1, 1.0), (2, 0.58), (3, 0.25), (4, 0.10), (5, 0.04),
            (6, 0.02),
        ],
        ...
        pitch_decay2_mult=1.3,
        filter_cutoff=3200,
        noise_level=0.004, noise_decay=50.0,
        liveliness=0.004,
    ),
...
def build_envelope(timbre, duration, n_samples, velocity, frequency):
    ...
    if d2_n > 0:
        ...
        env[d2_start:r_start] = sl * np.exp(
            -np.linspace(0, d2_n / SAMPLE_RATE, d2_n) / tau)
...
def synthesize(frequency, duration, velocity, timbre, timbre_name="default"):
    ...
    if timbre.detune_cents > 0 and timbre.detune_mix > 0:
        ...
        waveform += timbre.detune_mix * (
            np.sin(2*np.pi*frequency*ratio*t) + np.sin(2*np.pi*frequency/ratio*t))
...
def generate_room_ir(duration=0.9, room_size=0.40, damping=0.65):
    ...
    if ln > 0:
        ...
        np.random.seed(42); ir_l[ls:] += sosfilt(sos, np.random.randn(ln)*env*0.06)
        np.random.seed(137); ir_r[ls:] += sosfilt(sos, np.random.randn(ln)*env*0.06)
...
def stereo_compressor(left, right, threshold=0.55, ratio=2.5, attack_ms=5, release_ms=60):
    ...
    env = np.empty(len(linked)); env[0] = linked[0]
...
def collect_tempo_map(mid):
    ...
    for tr in mid.tracks:
        ...
        for msg in tr:
            tk += msg.time
            if msg.type == "set_tempo": ev.append((tk, msg.tempo))
    ev.sort(key=lambda x: x[0])
    if not ev or ev[0][0] != 0: ev.insert(0, (0, 500000))
    return ev

def ticks_to_seconds(abs_ticks, tempo_map, tpb):
    sec = 0.0; pt = 0; pp = tempo_map[0][1]
    for tk, tp in tempo_map[1:]:
        if tk >= abs_ticks: break
        sec += (tk-pt)/tpb*(pp/1e6); pt = tk; pp = tp
    ...
    for track in mid.tracks:
        ...
        for msg in track:
            ...
            if msg.type == "program_change":
                ...
                if key in active:
                    st = active.pop(key); vl = active_vel.pop(key)
...
def render_tracks(tracks, output_file):
    ...
    for notes in tracks:
        for start,note,dur,vel,ch,prog in notes:
            e = int((start+dur)*SAMPLE_RATE)+SAMPLE_RATE
    ...
    for ti, notes in enumerate(tracks):
        ...
        for start,note,dur,vel,ch,prog in notes:
            ...
            e = s + len(tone)
            if e > max_end: tone = tone[:max_end-s]; e = max_end
            buf[s:e] += tone

        if sb > 0: buf = apply_soundboard(buf, sb)

        rms = np.sqrt(np.mean(buf**2))
        if rms > 1e-6: buf *= 0.12 / rms
    ...
    if len(tracks) > 0:
        ...
        master_l *= 0.92/pk; master_r *= 0.92/pk
...
def main():
    ...
    a = p.parse_args()
    render_tracks(parse_midi(a.input), a.output or os.path.splitext(a.input)[0]+".flac")
