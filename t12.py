...
TIMBRES = {
    "piano": Timbre(
        ...
        decay2=0.6, decay2_slow=2.5, prompt_ratio=0.75,
        release=0.50,
...
def build_envelope(timbre, duration, n_samples, velocity, frequency, note_off_sample=None):
    ...
    if r_start < n_samples:
        ...
        env[r_start:] = sv * np.exp(-5.0 * np.linspace(0, 1, r_len))
...
def synthesize(frequency, duration, velocity, timbre, timbre_name="default", note_id=0):
    ...
    if timbre.detune_cents > 0 and timbre.detune_mix > 0:
        ...
        h0_env = h0_env_cached if h0_env_cached is not None else build_harmonic_envelope(timbre, total_dur, n_samples, 0, frequency, note_off)
        if timbre.vibrato_depth > 0:
            vib_env_d = np.clip((t - timbre.vibrato_delay) / 0.2, 0, 1)
            vib_d = vib_env_d * timbre.vibrato_depth * np.sin(2 * np.pi * timbre.vibrato_rate * t)
            inst_freq = frequency * (2 ** (vib_d / 1200.0))
            dp_up = 2 * np.pi * np.cumsum(inst_freq * ratio) / SAMPLE_RATE
            dp_dn = 2 * np.pi * np.cumsum(inst_freq / ratio) / SAMPLE_RATE
            waveform += timbre.detune_mix * (np.sin(dp_up) + np.sin(dp_dn)) * h0_env
        else:
            waveform += timbre.detune_mix * (
                np.sin(2 * np.pi * frequency * ratio * t) +
                np.sin(2 * np.pi * frequency / ratio * t)) * h0_env
