...
def generate_hrtf_pair(azimuth_deg, ir_length=64):
    ...
    g_near = 10 ** (abs(1.7 * np.sin(az)) / 20)
    g_far = 10 ** (-abs(1.7 * np.sin(az)) / 20)
...
def generate_room_ir(duration=1.4, room_size=0.50, damping=0.55):
    ...
    for i, (dms, g) in enumerate(zip(delays_ms, gains)):
        d_l = int(SAMPLE_RATE * dms * room_size / 1000)
        d_r = d_l + int(SAMPLE_RATE * (0.0002 + 0.0006 * rng_ir.random()))
        gpan = 0.05 * np.sin(i * 2.3 + 0.7)
        if d_l < n:
            ir_l[d_l] += g * (1.0 + gpan)
        if d_r < n:
            ir_r[min(d_r, n - 1)] += g * (1.0 - gpan)
...
def apply_conv_reverb(left, right, ir_l, ir_r, wet=0.08, predelay_ms=5):
