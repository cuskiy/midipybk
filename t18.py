...
# Azimuth angles — simulates orchestra seating.
# Wider spread than t16; duplicates get alternating offsets.
TRACK_PANNING = {
    "piano": -5, "electric_piano": -12, "organ": 5, "harpsichord": -8,
    "guitar": -25, "nylon_guitar": -20, "bass": 0,
    "strings": 22, "cello": 15, "brass": 30, "woodwind": -22, "flute": -18,
    "choir": 0, "celesta": 12, "vibraphone": -16, "marimba": -12, "harp": 18,
    "synth_pad": 0, "synth_lead": 10, "pluck": -14, "default": 0, "drums": 0,
}
...
def synthesize(frequency, duration, velocity, timbre, timbre_name="default", note_id=0):
    ...
    for idx, (mult, amp) in enumerate(timbre.harmonics):
        ...
        harm_sig = np.sin(phase * actual_mult)
        if det_ratio > 0:
            harm_sig += timbre.detune_mix * (
                np.sin(phase * actual_mult * det_ratio) +
                np.sin(phase * actual_mult / det_ratio))
        if h_env is not None:
            harm_sig *= h_env
        waveform += h_amp * harm_sig
...
def generate_hrtf_pair(azimuth_deg, ir_length=64):
    ...
    g_near = 10 ** (abs(2.2 * np.sin(az)) / 20)
    g_far = 10 ** (-abs(2.2 * np.sin(az)) / 20)
    near_ir = np.zeros(ir_length)
    far_ir = np.zeros(ir_length)
    near_ir[0] = 1.0
    d_samp = itd * SAMPLE_RATE
    d0 = min(int(d_samp), ir_length - 2)
    frac = d_samp - d0
    far_ir[d0] = 1.0 - frac
    if d0 + 1 < ir_length:
        far_ir[d0 + 1] = frac
    sfc = 8000 - 3000 * abs(np.sin(az))
    if sfc < SAMPLE_RATE * 0.45:
        far_ir = sosfilt(butter(2, sfc, btype='low', fs=SAMPLE_RATE, output='sos'), far_ir)
    near_ir *= g_near
    far_ir *= g_far
...
def generate_room_ir(duration=1.4, room_size=0.50, damping=0.55):
    n = int(SAMPLE_RATE * duration)
    ir_l, ir_r = np.zeros(n), np.zeros(n)

    delays_ms = [1.5, 4.2, 8.0, 13.5, 20.0, 28.0, 38.0, 52.0]
    gains = [0.50, 0.36, 0.25, 0.16, 0.10, 0.06, 0.035, 0.018]
    rng_ir = np.random.RandomState(314)
    for i, (dms, g) in enumerate(zip(delays_ms, gains)):
        jit_l = 1.0 + 0.12 * (rng_ir.random() - 0.5)
        jit_r = 1.0 + 0.12 * (rng_ir.random() - 0.5)
        d_l = int(SAMPLE_RATE * dms * jit_l * room_size / 1000)
        d_r = int(SAMPLE_RATE * dms * jit_r * room_size / 1000)
        pan = 0.10 * np.sin(i * 2.3 + 0.7)
        if d_l < n:
            ir_l[d_l] += g * (1.0 + pan)
        if d_r < n:
            ir_r[min(d_r, n - 1)] += g * (1.0 - pan)
    ...
    if ln > 0:
        ...
        ir_l[ls:] += sosfilt(sos, rng_l.randn(ln) * env * 0.07)
        rng_r = np.random.RandomState(137)
        ir_r[ls:] += sosfilt(sos, rng_r.randn(ln) * env * 0.07)

    pk = max(np.max(np.abs(ir_l)), np.max(np.abs(ir_r)), 1e-10)
    return ir_l / pk, ir_r / pk


def apply_conv_reverb(left, right, ir_l, ir_r, wet=0.08, predelay_ms=10):
    pd = int(SAMPLE_RATE * predelay_ms / 1000)
    if pd > 0:
        ir_l = np.concatenate([np.zeros(pd), ir_l])
        ir_r = np.concatenate([np.zeros(pd), ir_r])
    dry = 1.0 - wet
    return (left * dry + fftconvolve(left, ir_l, mode='full')[:len(left)] * wet,
            right * dry + fftconvolve(right, ir_r, mode='full')[:len(right)] * wet)


def stereo_compressor(left, right, threshold=0.70, ratio=1.5, attack_ms=20, release_ms=80):
    block = 64
    n = len(left)
    linked = np.maximum(np.abs(left), np.abs(right))

    n_blocks = (n + block - 1) // block
    block_env = np.empty(n_blocks)
    for i in range(n_blocks):
        s = i * block
        block_env[i] = np.max(linked[s:min(s + block, n)])

    ca = np.exp(-block / max(SAMPLE_RATE * attack_ms / 1000, 1))
    cr = np.exp(-block / max(SAMPLE_RATE * release_ms / 1000, 1))
    smooth = np.empty(n_blocks)
    smooth[0] = block_env[0]
    for i in range(1, n_blocks):
        c = ca if block_env[i] > smooth[i - 1] else cr
        smooth[i] = c * smooth[i - 1] + (1 - c) * block_env[i]

    env = np.repeat(smooth, block)[:n]
    gain = np.ones(n)
    m = env > threshold
...
def render_tracks(tracks, track_pans, output_file):
    ...
    for ti, notes in enumerate(tracks):
        ...
        # Peak-limit only — no RMS normalization. Preserves dynamics between tracks.
        ...
        if pan_cc is not None:
            ...
            if is_drum:
                ...
                if az in used_azimuths:
                    for offset in [8, -8, 16, -16, 24, -24]:
                        candidate = az + offset
                        if abs(candidate) <= 60 and candidate not in used_azimuths:
                            az = candidate
                            break
                used_azimuths.add(az)
    ...
    if len(tracks) > 0:
        # Reverb first, then gentle compression
        master_l, master_r = apply_conv_reverb(master_l, master_r, ir_l, ir_r, wet=0.08)
